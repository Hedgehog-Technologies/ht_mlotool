...
2. Open the RPF Explorer
   - `Tools... -> RPF Explorer...`
3. Once the RPF Explorer is loaded up, open up a folder that you want the output files to go to
   - `File -> Open Folder...`
   - *Note:* This doesn't have to be the final destination of the files
4. With that folder open in the RPF Explorer turn on `Edit Mode` by pressing the `Edit Mode` button near the top of the RPF Explorer window
5. With `Edit Mode` enabled, manually drag the .xml files that were generated by the MLO Tool from Windows Explorer to the RPF Explorer window
   - If done correctly, CodeWalker should now be showing the files as `.dat151.rel` and `.ymt` instead of `.xml`
6. You can now use these files as described in steps 10 and 11 in the [Usage Steps](#usage-steps) above
